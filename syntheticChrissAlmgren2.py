...
class MarketEnvironment():
    ...
    def trading_rate_action(self, action):
        """Convert network output to executable trade size with market adaptation
    
        Args:
            action (float): Network output in [0,1] range
            
        Returns:
            float: Number of shares to sell this step
            
        Strategy:
            1. Calculate baseline average rate
            2. Adjust for market volatility and liquidity
            3. Apply soft saturation to prevent extreme trades
            4. Enforce remaining position constraints
        """
        # Base average rate (TWAP equivalent)
        avg_rate = self.total_shares / self.num_n
        remaining_trades = max(1, self.timeHorizon)
        
        # Market condition factors
        volatility_factor = 1 + 5 * np.std(list(self.logReturns))  # Recent volatility
        liquidity_factor = 1 / (1 + self.shares_remaining/DAILY_TRADE_VOL)
        
        # Adaptive boundary parameters
        max_multiplier = min(
            3.0,  # Absolute maximum
            1.0 + 2 * (self.k/self.num_n),  # Increase aggression as time progresses
            1.0 + (self.shares_remaining/(self.total_shares/remaining_trades))  # Inventory pressure
        )
        
        # Dynamic maximum rate calculation
        dynamic_max = avg_rate * max_multiplier * volatility_factor * liquidity_factor
    
        # Final action scaling with soft boundaries
        return dynamic_max * (1 - np.exp(-5 * action))  # Soft saturation curve
    ...
    def improved_reward(self, info):
        """
        Optimized reward function targeting IS reduction while balancing multiple objectives.
        
        Components (all normalized by initial portfolio value):
            1. Impact Penalty           : Penalize immediate (permanent + temporary) impact costs.
            2. Progress Bonus           : Reward partial progress (non-linear for early trades).
            3. Risk Penalty             : Penalize large remaining position (variance-based).
            4. Benchmark (TWAP) Bonus   : Reward outperformance vs. a simple TWAP schedule.
            5. Time Efficiency          : Encourage timely trading without waiting too long.
            6. Slippage Penalty         : Asymmetric penalty for negative vs. positive slippage.
            
        Terminal Adjustments:
            - Implementation Shortfall Penalty (log-based)
            - Early Completion Bonus (time-based)
            
        Returns:
            A step or final reward (unscaled). The final scaling/clipping 
            to [-1,1] happens in calculate_reward(...).
        """
       

        initial_value = self.total_shares * self.startingPrice
        norm_scale = 1.0 / initial_value # Normalization factor

        # ---- Core Components ---- #

        # ---------------------------------------------------------------------
        # 1) Immediate Impact (Permanent + Temporary)
        #    - Usually positive cost => negative reward
        # ---------------------------------------------------------------------
        # impact_cost = (info.currentPermanentImpact + info.currentTemporaryImpact) * info.share_to_sell_now
        # # We'll use log(1 + cost) style, but ensure cost >= 0 for the penalty. 
        # # If impact_cost < 0 somehow, it implies a "negative cost" which would be a benefit, 
        # # but that’s unusual. For safety, we handle sign:
        # signed_impact = abs(impact_cost)
        # # More cost => bigger negative penalty. log1p is smooth.
        # # Multiplying by 5 to scale up the penalty.
        # impact_penalty = -5.0 * np.log1p(signed_impact * norm_scale)
        # # 1. Immediate Impact Cost (permanent + temporary)
        impact_cost = (info.currentPermanentImpact + info.currentTemporaryImpact) * info.share_to_sell_now
        impact_penalty = 20 * impact_cost * norm_scale
        # print('i',impact_penalty)

        # 2. Progress Incentive (non-linear reward for shares sold)
        # progress = (self.total_shares - self.shares_remaining) / self.total_shares
        # progress_bonus = 2 * np.sqrt(progress)  # Reward early progress

        # ---------------------------------------------------------------------
        # 3) Risk Penalty (remaining shares => variance)
        #    - Large leftover position => bigger penalty
        # ---------------------------------------------------------------------
        risk_penalty =  - self.llambda * self.singleStepVariance * (self.shares_remaining**2) * norm_scale * 2
        # print('r',risk_penalty)

        # ---------------------------------------------------------------------
        # 4) Benchmark (TWAP) Outperformance
        #    - Compare totalCapture to a naive TWAP assumption.
        # ---------------------------------------------------------------------
        # A simple approximate TWAP baseline: (remaining fraction) of initial_value 
        # or you can do a more direct approach. 
        # Here, user attempts:
        twap_expected = self.startingPrice * self.total_shares * self.k/self.num_n
        outperformance = (self.totalCapture - twap_expected) * norm_scale
        benchmark_bonus = 2 * outperformance  
        # print('b',benchmark_bonus)

        # ---------------------------------------------------------------------
        # 5) Time Efficiency
        #    - Encourage not waiting until last second
        # ---------------------------------------------------------------------
        # time_weight = (self.num_n - self.timeHorizon)/self.num_n  # [0-1]
        # time_efficiency = 1.5 * (1 - time_weight**2)  # Decaying bonus for early steps


        # ---------------------------------------------------------------------
        # 6) Slippage Penalty 
        #    - Asymmetric penalty for negative vs. positive slippage
        # ---------------------------------------------------------------------
        # Slippage = (exec_price - mid_price) / mid_price. If negative => we got a worse fill.
        slippage = (info.exec_price - info.price) * info.share_to_sell_now
        slippage_penalty = 20 * slippage * norm_scale
        # print('s', slippage_penalty)
        # slippage_penalty = -10 * np.where(
        #     slippage < 0, 
        #     abs(slippage)**1.5,  # Strong penalty for negative slippage
        #     abs(slippage)         # Mild penalty for positive slippage
        # )


        # ---------------------------------------------------------------------
        # If we are done, add final shortfall penalty & possible early bonus
        # ---------------------------------------------------------------------
        if info.done:
            # Final IS penalty with progressive scaling
            # = (initial_value) - totalCapture
            # Usually >= 0, but can be negative if we beat the baseline (price > start).
            isf = (self.total_shares * self.startingPrice) - self.totalCapture
            
            is_penalty = -isf * norm_scale * 20
        
            # Early completion bonus (linear time reward)
            # time_bonus = 2 * (1 - self.k/self.num_n)

            return (is_penalty + benchmark_bonus + 
                    impact_penalty  + slippage_penalty)

        # print('i/',impact_penalty/(abs(impact_penalty) + abs(slippage_penalty) + abs(benchmark_bonus) + abs(risk_penalty)))
        # print('s/',slippage_penalty/(abs(impact_penalty) + abs(slippage_penalty) + abs(benchmark_bonus) + abs(risk_penalty)))
        # print('b/',benchmark_bonus/(abs(impact_penalty) + abs(slippage_penalty) + abs(benchmark_bonus) + abs(risk_penalty)))
        # print('r/',risk_penalty/((abs(impact_penalty) + abs(slippage_penalty) + abs(benchmark_bonus) + abs(risk_penalty))))
        # ---------------------------------------------------------------------
        # Otherwise, regular step reward
        # ---------------------------------------------------------------------
        # ---- Regular Step Reward ---- #
        return (impact_penalty + slippage_penalty + benchmark_bonus + risk_penalty)

    def calculate_reward(self, info):
        """
        Final reward scaling with dynamic factors, clipped to [-1, 1].
        """
        raw_reward = self.improved_reward(info)

        # Market condition scaling
        volatility_factor = 1 + 2 * np.std(list(self.logReturns))
        liquidity_factor = 1 - 0.5 * (info.share_to_sell_now / DAILY_TRADE_VOL)

        # Time-dependent scaling
        time_scale = 1 + 2 * (self.k / self.num_n)  # Increase reward sensitivity over time

        # Multiply all factors
        scaled_reward = raw_reward * time_scale * volatility_factor * liquidity_factor

        # Soft clip to [-10, 10], then scale to [-1, 1]
        clipped = np.clip(scaled_reward, -10.0, 10.0)
        final = clipped / 10.0  # => [-1, 1]
